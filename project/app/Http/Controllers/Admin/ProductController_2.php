...
class ProductController extends Controller
{
    ...
    public function importSubmit(Request $request)
    {
        ...
        while (($line = fgetcsv($file,1000,';')) !== FALSE) {
            if($i != 1)
            {
                ...
                if (Storage::where(['storage_ID'=>utf8_encode($line[4])])->count()==0 || Storage::where(['storage_ID'=>utf8_encode($line[4])])->count()==null){
                    //--- Validation Section Ends
                    ...
                    $hotelstorage = HotelStorage::where("customer_name", utf8_encode($line[0]));

                    if($hotelstorage->count()!=0 || $hotelstorage->count()!=null){
                        $input_data['hotel_id'] = $hotelstorage->first()->hotel_id;
                        $input_data['storage_ID'] = utf8_encode($line[4]);
                        $input_data['location'] = utf8_encode($line[5]);
                        $input_data['quantity'] = $quantity;
                        $input_data['tire_size'] = utf8_encode($line[9]);
                        $input_data['tire_brand'] = utf8_encode($line[11]);
                        $input_data['weather'] = utf8_encode($line[12])=='S'?1:0;
                        $input_data['is_rim'] = utf8_encode($line[13])=='Yes'?1:0;
                        $input_data['rim_size'] = utf8_encode($line[10]);
                        $input_data['photos'] = 'upload.png';
                        $input_data['car_license'] = utf8_encode($line[3]);
                        $input_data['car_make'] = utf8_encode($line[7]);
                    ...
                    // Save Data
                        ...
                        $log_arr['staff_id'] = Auth::guard("admin")->user()->id;
                        $log_arr['content'] = 'Newly Registered by Importing';
                        $log = Log::create($log_arr);

                    }
                    else{
                        $data_hotel = new HotelStorage;
                        $input_hotel['customer_name'] = utf8_encode($line[0]);
                        $input_hotel['social_id'] = null;
                        $input_hotel['home_address'] = utf8_encode($line[1]);
                        $input_hotel['phone'] = utf8_encode($line[2]);
                        ...
                        $hotelstorage = HotelStorage::where("customer_name", utf8_encode($line[0]))->first();
                ...
                else
                {
                    ...
                    $input_data = array();
                    $input_hotel = array();
                    $exist_data = Storage::where(['storage_ID'=>utf8_encode($line[4])])->first();
                    $exist_hotel_id = $exist_data->hotel_storage->hotel_id;
                        ...
                        $input_data['created_at'] = $newDate;
                        $input_data['staff_id'] = Auth::guard("admin")->user()->id;
                    $update_data = Storage::where('storage_ID','=',utf8_encode($line[4]))
                                            ->update($input_data);
                    ...
                    $log_arr['content'] = 'Update Existed data by Importing';
                    $log_arr['staff_id'] = Auth::guard("admin")->user()->id;
                    $log = Log::create($log_arr);
                }
    ...
    public function exportSubmit(Request $request){
        ...
        $columns = array('Name', 'Home address', 'Mobile phone', 'Car numberplate', 'Storage ID', 'Storage Location', 'Date', 'Car', 'Qty', 'Size', 'Rim Size', 'Tire brand', 'S/V', 'On Rims','status');
        $callback = function() use($storage_data, $columns) {
            $file = fopen('php://output', 'w');
            fputcsv($file, $columns,';');
            
            foreach($storage_data as $storage) {
                $weather = $storage->weather==1?'s':'w';
                $rim = $storage->is_rim == 1?'Yes':'Nei';
                $status='';
                ...
                fputcsv($file, array($storage->hotel_storage->customer_name,$storage->hotel_storage->home_address,$storage->hotel_storage->phone,$storage->car_license,$storage->storage_ID,$storage->location,$storage->created_at,$storage->car_make,$storage->quantity,$storage->tire_size,$storage->rim_size,$storage->tire_brand,$weather,$rim,$status),';');
            }
            fclose($file);
        };
